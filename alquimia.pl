...
tiene(ana, tierra).
tiene(ana, hierro).
% beto tiene todo lo que tiene ana
tiene(beto, Elemento) :-
    tiene(ana, Elemento).
tiene(cata, fuego).
tiene(cata, agua).
...
% Para los elementos vivos sirve el libro de la vida (y para los no vivos el libro inerte). 
% Además, las cucharas y círculos sirven cuando soportan la cantidad de ingredientes del elemento 
%(las cucharas tantos ingredientes como centímetros/10, y los círculos alquímicos tantos ingredientes como metros * cantidad de niveles).
